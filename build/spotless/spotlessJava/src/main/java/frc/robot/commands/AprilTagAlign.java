...
public class AprilTagAlign extends Command {
  ...
  public void execute() {
    ...
    m_turnPower =
        m_controller.calculate(m_vision.getX())
            + Math.copySign(
                Constants.VisionConstants.kSTurn,
                m_controller.calculate(m_vision.getX())); // Calculate P value
    ...
    CommandScheduler.getInstance()
        .schedule(DriveCommands.joystickDrive(m_drivetrain, null, null, null));
    ;

    // m_drivetrain.runVelocity(ChassisSpeeds.fromFieldRelativeSpeeds(m_turnError, m_turnPower,
    // m_turnError, null))

    // m_drivetrain.drive(
    //   MathUtil.applyDeadband(-m_translator.getY()*OperatorConstants.translationMultiplier,
    // OperatorConstants.kDriveDeadband),
    //   MathUtil.applyDeadband(-m_translator.getX()*OperatorConstants.translationMultiplier,
    // OperatorConstants.kDriveDeadband),
    //   (m_turnPower),
    //   true, true);
    CommandScheduler.getInstance()
  ...
  public void end(boolean interrupted) {
    CommandScheduler.getInstance()
        .schedule(DriveCommands.joystickDrive(m_drivetrain, () -> 0, () -> 0, () -> 0));
  }
